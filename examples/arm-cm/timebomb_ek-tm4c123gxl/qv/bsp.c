    ...
    */
...
#if (Q_PARAM_SIZE != 0)
            QACTIVE_POST(AO_TimeBomb, BUTTON_PRESSED_SIG, 0U);
#else
            QACTIVE_POST(AO_TimeBomb, BUTTON_PRESSED_SIG);
#endif
        }
        else { /* the button is released */
            /* post the "button-released" event from ISR */
#if (Q_PARAM_SIZE != 0)
            QACTIVE_POST(AO_TimeBomb, BUTTON_RELEASED_SIG, 0U);
#else
            QACTIVE_POST(AO_TimeBomb, BUTTON_RELEASED_SIG);
#endif
        }
    }
    if ((tmp & BTN_SW2) != 0U) {  /* debounced SW2 state changed? */
        if ((buttons.depressed & BTN_SW2) != 0U) { /* is SW2 depressed? */
            /* post the "button-pressed" event from ISR */
#if (Q_PARAM_SIZE != 0)
            QACTIVE_POST(AO_TimeBomb, BUTTON2_PRESSED_SIG, 0U);
#else
            QACTIVE_POST(AO_TimeBomb, BUTTON2_PRESSED_SIG);
#endif
        }
        else { /* the button is released */
            /* post the "button-released" event from ISR */
#if (Q_PARAM_SIZE != 0)
            QACTIVE_POST(AO_TimeBomb, BUTTON2_RELEASED_SIG, 0U);
#else
            QACTIVE_POST(AO_TimeBomb, BUTTON2_RELEASED_SIG);
#endif
        }
    }
}
